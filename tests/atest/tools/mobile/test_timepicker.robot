*** Settings ***
Documentation    E2E tests for Time picker - Happy paths
Library    AppiumLibrary
Library    Agent
Library    DateTime
Resource    ${EXECDIR}/tests/atest/config/devices.robot

*** Keywords ***
Get Device Time In 12H Format
    [Documentation]    Returns device time in 12-hour format with AM/PM (e.g., "3:27 AM")
    ${time_12h} =    Get Device Time    h:mm A
    Log    Device time (12h format): ${time_12h}
    RETURN    ${time_12h}

Get Device Time Plus Hours
    [Documentation]    Returns device time with added hours in 12-hour format with AM/PM
    [Arguments]    ${hours}
    # Get current datetime and add hours
    ${current_datetime} =    Get Current Date
    ${new_datetime} =    Add Time To Date    ${current_datetime}    ${hours} hours
    ${new_time_12h} =    Convert Date    ${new_datetime}    result_format=%I:%M %p
    
    Log    New time (+ ${hours}h, 12h format): ${new_time_12h}
    RETURN    ${new_time_12h}

Get Device Time Plus Minutes
    [Documentation]    Returns device time with added minutes in 12-hour format with AM/PM
    [Arguments]    ${minutes}
    # Get current datetime and add minutes
    ${current_datetime} =    Get Current Date
    ${new_datetime} =    Add Time To Date    ${current_datetime}    ${minutes} minutes
    ${new_time_12h} =    Convert Date    ${new_datetime}    result_format=%I:%M %p
    
    Log    New time (+ ${minutes}m, 12h format): ${new_time_12h}
    RETURN    ${new_time_12h}

*** Test Cases ***
Test Time Picker Display Positif - Correct Time Correct AM/PM
    [Documentation]    Happy path: Select a time in the Time picker
    [Tags]    mobile    pickers    time
    Open Application With Config
    Sleep    1s
    
    Agent.do    instruction=click on the TIME button
    Sleep    1s
    
    Agent.check    instruction=verify that the Time picker is displayed
    Sleep    1s
    
    ${device_time} =    Get Device Time In 12H Format
    ${device_time_delta} =    Get Device Time Plus Minutes    1
    Agent.check    instruction=verify that time displayed on the screen is between ${device_time} and ${device_time_delta} without caring about the AM/PM
    Sleep    1s
    
    Close Application

Test Time Picker Display Negative - Correct Time Wrong AM/PM
    [Documentation]    Negative test: Verify wrong time is displayed
    [Tags]    mobile    pickers    time    negative   high_precision
    Open Application With Config
    Sleep    1s
    
    Agent.do    instruction=click on the TIME button
    Sleep    1s
    
    Agent.check    instruction=verify that the Time picker is displayed
    Sleep    1s
    
    ${device_time} =    Get Device Time In 12H Format
    Run Keyword And Expect Error    *    Agent.check    instruction=verify that time displayed on the screen is ${device_time}
    Sleep    1s
    
    Close Application

Test Time Picker Display Negative - Wrong Time 
    [Documentation]    Negative test: Verify wrong time is NOT displayed
    [Tags]    mobile    pickers    time    negative
    Open Application With Config
    Sleep    1s
    
    Agent.do    instruction=click on the TIME button
    Sleep    1s
    
    Agent.check    instruction=verify that the Time picker is displayed
    Sleep    1s
    
    # Get device time + 1 hour (wrong time) in 12h format
    ${wrong_time} =    Get Device Time Plus Hours    1
    
    Run Keyword And Expect Error    *    Agent.check    instruction=verify that the time displayed on the screen is ${wrong_time} without caring about the AM/PM
    Sleep    1s
    
    Close Application